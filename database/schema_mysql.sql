# Profile Syntaxtrust - MySQL Database Schema

## Overview
This document provides the complete MySQL-compatible database schema for the Profile Syntaxtrust project, including table definitions, constraints, indexes, and relationships.
...
CREATE TABLE users (
    ...
    phone VARCHAR(20),
    foto VARCHAR(255), -- Profile photo path/URL
...
-- Database comment
ALTER DATABASE profile_syntaxtrust COMMENT = 'Database for Profile Syntaxtrust - Academic Web Development Services';
...
-- Query to check table status
SHOW TABLE STATUS LIKE 'profile_syntaxtrust%';

-- =====================================================
-- BACKUP AND MAINTENANCE
-- =====================================================

-- Create a backup (run this command in terminal)
-- mysqldump -h localhost -u root -p profile_syntaxtrust > /path/to/backup/profile_syntaxtrust_$(date +%Y%m%d_%H%M%S).sql
...
ANALYZE TABLE service_categories;
ANALYZE TABLE services;
ANALYZE TABLE pricing_packages;
ANALYZE TABLE users;
ANALYZE TABLE portfolio_items;
ANALYZE TABLE service_images;
...
- **TIMESTAMP** fields without time zone (MySQL default behavior)
- **JSON columns** for array-like data storage
- **ENUM types** for status and category fields
- **Standard MySQL syntax** throughout
...
- **INT AUTO_INCREMENT** primary keys for large datasets
- **JSON fields** for flexible feature lists
- **Proper indexing** strategy
- **Query optimization** ready

This MySQL schema provides a production-ready foundation for the Profile Syntaxtrust application with proper relationships, constraints, and optimization for academic web development services.
